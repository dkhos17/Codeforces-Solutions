...
/*
 * Problem: 1181B Split-a-Number
 * Author: Harold-Finch
 * Date: Sunday, June 16, 2019 2:18 PM
 * Status: Accepted, 46 ms, 614400 bytes
 * Language: GNU C++14
*/
...
int main() {
	...
	for(auto id : idx){
		if(s[id] == '0') continue;
		string frst = s.substr(0,id);
		string sec = s.substr(id,n);
		if(frst.size() == 0 || sec.size() == 0) continue;
		bigint sum = bigint(frst) + bigint(sec);
		if(res > sum) res = sum;
	}
